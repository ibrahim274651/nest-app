...
import { LookupOptions } from 'src/pipeline';
...
export class PromotionsService {
  ...
  async findAll(pageOptionsDto: PageOptionsDto) {
    ...
    try {
      ...
      const paginatedResults = filteredResults.slice(skip, skip + (take ?? 10));
    ...
    return results.filter((item) => {
      ...
      return (
        item.articles.some((article) => {
          ...
          // Check if articleId is populated with Article document
          if (
            articleId &&
            typeof articleId === 'object' &&
            'designation' in articleId &&
            '_id' in articleId
          ) {
            const id = String(articleId._id);
            const designation = String(articleId.designation);
            return id.match(regex) || designation.match(regex);
          }
          return false;
