...
export class AccompagnementsService {
  private readonly logger = new Logger(AccompagnementsService.name);
  constructor(
  ...
  // Validate accompagnements
  async validateAccompagnements(accompagnements?: string[]): Promise<any[]> {
    ...
    const existingAccompagnements = await this.addOnModel.find({
      _id: {
        $in: accompagnementsIds.map((id) => new mongoose.Types.ObjectId(id)),
      },
    });

    if (existingAccompagnements.length !== accompagnementsIds.length) {
      const existingIds = existingAccompagnements.map((item) =>
        item._id.toString(),
      );
      const missingIds = accompagnementsIds.filter(
        (id) => !existingIds.includes(id),
      );

      this.logger.error(
        `Les accompagnements suivants sont introuvables : ${missingIds.join(', ')}`,
      );
