...
export class ArticlesService {
  ...
  async findItemsByStore(store?: FilterStoreDto) {
    try {
      const filter: any = {};
      ...
      // console.log('Movement Data: ', JSON.stringify(results, null, 4));
  ...
  private async transformResults(results: any[]): Promise<any[]> {
    const prixTotalByTypeMouvement: { [key: string]: number } = {};
    ...
    return results.map((store) => {
      const transformedStore = {
        ...
        items: store.movements.flatMap((movement) =>
          movement.mvtDetailsData.map((detail) => {
            const itemId = detail.itemsData._id.toString();

            // Initialize quantites tracking for this article if not exists
            ...
            return {
              ...
              prixTotal:
                detail.itemsData.tarification.prixTTC * detail.quantite,
              ...
              tarification: {
                ...
                tar_designation:
                  detail.itemsData.tarification.tarificationId.designation,
                tar_modeConsommation:
                  detail.itemsData.tarification.tarificationId.modeConsommation,
      ...
      // Calculate the sum of prixTotal for the current typeMouvement
      const currentTypeMouvement = transformedStore.typeMouvement;
      const totalPrixTotal = transformedStore.items.reduce(
        (sum, item) => sum + item.prixTotal,
        0,
      );

      // Update the totalPrixTotalByTypeMouvement object
